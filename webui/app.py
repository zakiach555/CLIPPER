...
def run_viral_cutter(input_source, project_name, url, video_file, segments, viral, themes, min_duration, max_duration, model, ai_backend, api_key, ai_model_name, chunk_size, workflow, face_model, face_mode, face_detect_interval, no_face_mode, 
    ...
    if use_custom_subs:
        ...
        subtitle_config["speaker_duration"] = float(speaker_duration) if speaker_duration else 0.0
    ...
    with gr.Tabs():
        with gr.Tab(i18n("Create New")):
             with gr.Row():
                with gr.Column(scale=1):
                    ...
                    with gr.Row():
                        ...
                        translate_input = gr.Dropdown(choices=["None", "ar", "pt", "en", "es", "fr", "de", "it", "ru", "ja", "ko", "zh-CN"], label=i18n("Translate Subtitles To"), value="None")
             ...
             with gr.Accordion(i18n("Subtitle Settings (alpha)"), open=False):
                preset_input = gr.Dropdown(choices=[(i18n("Manual"), "Manual")] + [(i18n(k), k) for k in subs.SUBTITLE_PRESETS.keys()], label=i18n("Quick Presets"), value="Arabic Gold")
                ...
                with gr.Accordion(i18n("Advanced Settings"), open=False):
                    ...
                    with gr.Row():
                        font_name_input = gr.Textbox(label=i18n("Font Name"), value="Lyon Arabic Display Black")
                        font_size_input = gr.Slider(label=i18n("Font Size (Base)"), minimum=8, maximum=80, value=18)
                        highlight_size_input = gr.Slider(label=i18n("Highlight Size"), minimum=8, maximum=80, value=34)

                    with gr.Row():
                        font_color_input = gr.ColorPicker(label=i18n("Base Color"), value="#FFFFFF")
                        highlight_color_input = gr.ColorPicker(label=i18n("Highlight Color"), value="#FFD700")
                    ...
                    with gr.Row():
                        outline_thickness_input = gr.Slider(label=i18n("Outline Thickness"), minimum=0, maximum=10, value=3)
                    ...
                    with gr.Row():
                        ...
                        gap_limit_input = gr.Slider(label=i18n("Gap Limit"), minimum=0.0, maximum=5.0, value=0.3, step=0.1)
                        mode_input = gr.Dropdown(choices=[(i18n("Highlight"), "highlight"), (i18n("Two Tier"), "two_tier"), (i18n("Word by Word"), "word_by_word"), (i18n("No Highlight"), "no_highlight")], label=i18n("Mode"), value="highlight")
                        words_per_block_input = gr.Slider(label=i18n("Words per Block"), minimum=1, maximum=20, value=3, step=1)

                with gr.Accordion(i18n("Speaker Tag (Zone A)"), open=False):
                    gr.Markdown(i18n("Displays a gold name + white title in the top-left corner for a set duration."))
                    with gr.Row():
                        speaker_name_input = gr.Textbox(label=i18n("Speaker Name"), placeholder=i18n("e.g. Ahmed Al-Rashid"))
                        speaker_title_input = gr.Textbox(label=i18n("Speaker Title"), placeholder=i18n("e.g. CEO, Motivational Speaker"))
                        speaker_duration_input = gr.Number(label=i18n("Duration (seconds)"), value=0, minimum=0, precision=1)
             ...
             start_btn.click(run_viral_cutter, inputs=[
                 ...
                 speaker_name_input, speaker_title_input, speaker_duration_input
