...
def download(url, base_root="VIRALS", download_subs=True, quality="best"):
    ...
    ydl_opts = {
        ...
        'no_warnings': False,
        'force_ipv4': True,
        # Retry / timeout settings — survive flaky connections and 429 bursts
        'retries': 15,
        'fragment_retries': 15,
    ...
    try:
        ...
        if "No address associated with hostname" in error_str or "Failed to resolve" in error_str:
            ...
            # Try up to 3 times to download the video (handles mid-download timeouts)
            last_err = None
            for attempt in range(1, 4):
                try:
                    ...
                    last_err = e2
                    err2 = str(e2)
                    if "timed out" in err2 or "Connection" in err2 or "429" in err2:
                        print(i18n("Attempt {}/{} failed ({}). Retrying...").format(attempt, 3, err2[:80]))
                    else:
                        print(i18n("Fatal error on attempt {}: {}").format(attempt, e2))
                        raise
            if last_err:
                print(i18n("All retry attempts failed: {}").format(last_err))
                raise last_err
