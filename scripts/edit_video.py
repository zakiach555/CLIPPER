...
def _test_encoder(codec, preset):
    """Return True if the encoder can actually encode a tiny test frame."""
    try:
        test = subprocess.run(
            ['ffmpeg', '-hide_banner', '-loglevel', 'error',
             '-f', 'lavfi', '-i', 'testsrc=duration=0.04:size=64x64:rate=25',
             '-vframes', '1', '-c:v', codec, '-preset', preset, '-f', 'null', '-'],
...
def get_best_encoder():
    ...
    try:
        ...
        # Priority: NVENC > AMF > QSV > VideoToolbox > CPU
        # Each candidate is tested before being accepted to avoid silent runtime failures.
        if "h264_nvenc" in output and _test_encoder("h264_nvenc", "fast"):
            print("Encoder Detected: NVIDIA (h264_nvenc)")
            CACHED_ENCODER = ("h264_nvenc", "fast")
            return CACHED_ENCODER

        if "h264_amf" in output and _test_encoder("h264_amf", "speed"):
            print("Encoder Detected: AMD (h264_amf)")
            CACHED_ENCODER = ("h264_amf", "speed")
            return CACHED_ENCODER

        if "h264_qsv" in output and _test_encoder("h264_qsv", "veryfast"):
            print("Encoder Detected: Intel QSV (h264_qsv)")
            CACHED_ENCODER = ("h264_qsv", "veryfast")
            return CACHED_ENCODER

        if "h264_videotoolbox" in output and _test_encoder("h264_videotoolbox", "default"):
    ...
    try:
        ...
        if detection_period is not None:
            if isinstance(detection_period, dict):
                val = detection_period.get('1', detection_period.get(1, 0.2))
                current_interval = max(1, int(float(val) * fps))
            else:
